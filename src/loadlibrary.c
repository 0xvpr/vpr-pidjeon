...
int inject_LoadLibraryA(DWORD process_id, const char* dll)
{
    ...
    LPVOID pLoadLibraryA = GetProcAddress(GetModuleHandle("kernel32.dll"), "LoadLibraryA");
...
int inject_LoadLibraryW(DWORD process_id, const char* dll)
{
    ...
    LPVOID pLoadLibraryW = GetProcAddress(GetModuleHandle("kernel32.dll"), "LoadLibraryW");
